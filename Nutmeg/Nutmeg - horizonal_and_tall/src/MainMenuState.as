package  
{
	...
	public class MainMenuState extends FlxState
	{
		...
		override public function create():void
		{
			...
			dolly = new FlxSprite(160, 1500);
			dolly.facing = FlxObject.UP;
		...
		override public function update():void
		{
			...
			if (dolly.facing == FlxObject.UP)
			{
				dolly.y--;
				
				if (dolly.y <= 0)
				{
					dolly.facing = FlxObject.DOWN;
				}
			}
			else if (dolly.facing == FlxObject.DOWN)
			{
				dolly.y++;
				
				if (dolly.y >= (level.height))
				{
					dolly.facing = FlxObject.UP;
				}
			}
