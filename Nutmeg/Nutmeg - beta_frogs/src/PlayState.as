package  
{
	...
	public class PlayState extends FlxState
	{
		...
		private function hitFrog(player:FlxObject, frog:FlxObject):void
		{
			if (Frog(frog).isDying)
			{
				return;
			}
			
			if ((player.y < frog.y) && (Player(player).isDying == false))
			{
				frog.kill();
				catDeathFX.play(true);
				if (FlxG.keys.SPACE)
